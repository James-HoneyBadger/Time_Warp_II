#!/usr/bin/env python3
"""
Syntax Highlighting and Line Numbers for Time Warp II
==================================================
...
# ---------------------------------------------------------------------------
# Custom Pygments lexer for the TempleCode language
# ---------------------------------------------------------------------------
if PYGMENTS_AVAILABLE:
    class TempleCodeLexer(RegexLexer):
        """Pygments lexer for TempleCode (BASIC + PILOT + Logo hybrid)."""
        name = 'TempleCode'
        aliases = ['templecode', 'tc']
        filenames = ['*.tc']

        tokens = {
            'root': [
                # Line numbers at start of line
                (r'^\d+', Token.Literal.Number.Integer),

                # REM / comments
                (r"(?i)\bREM\b.*$", Token.Comment.Single),

                # PILOT colon-commands (e.g. T:, A:, M:, Y:, N:, J:, C:, E:, R:)
                (r'(?i)^[TAMYNJCERX]:', Token.Keyword),
...
class SyntaxHighlightingText(tk.Frame):
    ...
    def _setup_syntax_highlighting(self):
        ...
        lexer_map = {
            'templecode': TempleCodeLexer,
            'text': TextLexer,
        }

        lexer_class = lexer_map.get(self.language.lower(), TempleCodeLexer)
        try:
    ...
    def find_text(self, search_term: str, start_pos: str = '1.0', case_sensitive: bool = False, 
        ...
        if regex:
            ...
            pattern = search_term if not whole_word else f'(?<!\\w){re.escape(search_term)}(?!\\w)'
